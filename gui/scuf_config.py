...
def main():
    ...
    trp = sub.add_parser('bootstrap-p1-test-target',
                         help='DIAGNOSTIC: replay p1_to_A sequence but override ONLY the '
                              '616d action_hdr byte-13.')
    trp.add_argument('target', metavar='TARGET', help='A B X Y LB RB')

    xmp = sub.add_parser('bootstrap-p1-xml-target',
                         help='Replay p1_to_A sequence but rebuild 606d XML with modified '
                              'P1→TARGET mapping. Also updates 616d byte-13 for consistency.')
    xmp.add_argument('target', metavar='TARGET', help='A B X Y LB RB')

    ftp = sub.add_parser('bootstrap-p1-full-target',
                         help='Replay with overrides for ALL target-dependent bytes '
                              '(606d XML, 616d byte-13, 0700 byte-2).')
    ftp.add_argument('target', metavar='TARGET', help='A B X Y LB RB')

    emp = sub.add_parser('bootstrap-p1-everything',
                         help='Full override + all preset-entry byte tweaks (62/66/67/68 6d).')
    emp.add_argument('target', metavar='TARGET', help='A B X Y LB RB')

    vrp = sub.add_parser('bootstrap-p1-verbatim',
                         help='Replay capture verbatim for specified target — NO '
                              'modifications. Uses sequence_p1_to_{target}.bin.')
    vrp.add_argument('target', metavar='TARGET', help='A B X Y LB RB')

    hrp = sub.add_parser('bootstrap-p1-hybrid',
                         help='Replay p1_to_A sequence skeleton with EXACT 606d bytes '
                              'from target-specific capture substituted in.')
    hrp.add_argument('target', metavar='TARGET', help='A B X Y LB RB')
    ...
    try:
        if args.cmd == 'show':
        ...
        elif args.cmd == 'bootstrap-p1-hybrid':
            tgt = args.target.upper()
            tgt_kn = TARGET_KEYS.get(tgt, tgt)
            code = _ACTION_HDR_TARGET_CODES.get(tgt_kn)
            if code is None:
                print(f'No target code for {tgt}', file=sys.stderr)
                sys.exit(1)

            # Load A sequence as skeleton
            seq_a = _load_p1_full_sequence(variant='A')
            # Load target sequence to extract its exact 606d bytes
            seq_t = _load_p1_full_sequence(variant=tgt)

            # Walk target sequence to reassemble 606d blob
            def extract_606d_from_seq(seq):
                blob = bytearray(); in_w = False; cs = ca = 0
                for _delay, pkt in seq:
                    if len(pkt) < 4: continue
                    cmd = pkt[2]; sub = pkt[3]
                    if cmd == 0x0d and sub == 0x01:
                        in_w = (pkt[4:6] == b'\x60\x6d')
                        if in_w:
                            blob = bytearray(); cs = ca = 0
                    elif in_w and cmd == 0x06 and sub == 0x01:
                        cs = struct.unpack_from('<H', pkt, 4)[0]
                        blob.extend(pkt[8:8+min(56, cs)])
                        ca = min(56, cs)
                    elif in_w and cmd == 0x07 and sub == 0x01:
                        rem = cs - ca
                        take = min(60, rem)
                        blob.extend(pkt[4:4+take])
                        ca += take
                return bytes(blob)

            tgt_606d = extract_606d_from_seq(seq_t)
            print(f'Target {tgt} 606d blob: {len(tgt_606d)}B')

            # Chunk target 606d at 512-byte boundaries
            CHUNK = 512
            tgt_chunks = [tgt_606d[i:i+CHUNK] for i in range(0, len(tgt_606d), CHUNK)]
            # Skeleton A's 606d chunk starts (indices 342, 351, 360, 369)
            chunk_start_indices = [342, 351, 360, 369]

            overrides = {}
            for chunk_idx, chunk in enumerate(tgt_chunks):
                if chunk_idx >= len(chunk_start_indices):
                    print(f'ERROR: target has more chunks than A skeleton supports')
                    sys.exit(1)
                base_idx = chunk_start_indices[chunk_idx]
                first = b'\x02\x08\x06\x01' + struct.pack('<H', len(chunk)) + b'\x00\x00' + chunk[:56]
                overrides[base_idx] = first.ljust(64, b'\x00')
                for j, off in enumerate(range(56, len(chunk), 60)):
                    cp = b'\x02\x08\x07\x01' + chunk[off:off+60]
                    overrides[base_idx + 1 + j] = cp.ljust(64, b'\x00')

            # 616d byte 13 (seq idx 378, data offset 8+13=21)
            orig_616 = bytearray(seq_a[378][1])
            orig_616[8 + 13] = code
            orig_616[8 + 14] = 0
            orig_616[8 + 15] = 0
            orig_616[8 + 16] = 0
            overrides[378] = bytes(orig_616)

            # 0700 manifest byte 2 (seq idx 439, data offset 8+2=10)
            orig_mf = bytearray(seq_a[439][1])
            orig_mf[8 + 2] = code
            overrides[439] = bytes(orig_mf)

            print(f'Replaying A skeleton with {len(overrides)} overrides for target {tgt}')
            _replay_p1_sequence(fd, overrides=overrides)
            print(f'Done. Restart controller, press P1 paddle, report result.')
        elif args.cmd == 'bootstrap-p1-verbatim':
            variant = args.target.upper()
            if variant not in ('A','B','X','Y','LB','RB'):
                print(f'Variant {variant} not supported', file=sys.stderr)
                sys.exit(1)
            print(f'Replaying capture p1_to_{variant} verbatim (no modifications)')
            seq = _load_p1_full_sequence(variant=variant)
            print(f'  Sequence length: {len(seq)} commands')
            # reuse _replay_p1_sequence but with loaded seq
            # inline the replay so we use our variant-specific sequence
            import fcntl, time as _time
            _send_write_unlock()
            fl = fcntl.fcntl(fd, fcntl.F_GETFL)
            fcntl.fcntl(fd, fcntl.F_SETFL, fl | os.O_NONBLOCK)
            try:
                ...
                fcntl.fcntl(fd, fcntl.F_SETFL, fl)
            print(f'Done. Restart controller, press P1 paddle, report result.')
        elif args.cmd == 'bootstrap-p1-everything':
            # Override EVERY target-dependent byte across captured slots:
            #   616d byte 13 = target_code
            #   0700 byte 2  = target_code
            #   686d byte 4  = preset_id     (varies per target family)
            #   626d byte 8  = preset_ent_1  (varies per target family)
            #   666d byte 8  = preset_ent_5  (varies per target family)
            #   676d byte 8  = preset_ent_6  (varies per target family)
            # "Family": A/LB identical, X/RB identical.
            TARGET_BYTES = {
                'GamepadFunctionButtonBottom': (0x1e, 0x16, 0x06, 0x02, 0x00),   # A
                'GamepadFunctionButtonRight':  (0x04, 0x12, 0x01, 0x06, 0x02),   # B
                'GamepadFunctionButtonLeft':   (0x06, 0x17, 0x07, 0x02, 0x03),   # X
                'GamepadFunctionButtonTop':    (0x08, 0x15, 0x00, 0x03, 0x06),   # Y
                'GamepadLeftBumper':           (0x12, 0x16, 0x06, 0x02, 0x00),   # LB
                'GamepadRightBumper':          (0x14, 0x17, 0x07, 0x03, 0x06),   # RB
            }
            tgt_kn = TARGET_KEYS.get(args.target.upper(), args.target)
            if tgt_kn not in TARGET_BYTES:
                print(f'Target {args.target} not in complete table', file=sys.stderr)
                sys.exit(1)
            code, b686, b626, b666, b676 = TARGET_BYTES[tgt_kn]

            # Build 606d chunks from modified XML
            orig_main = _load_p1_bootstrap_bundle()['606d']
            xml = zlib.decompress(orig_main[7:]).decode('utf-8', errors='replace')
            xml = xml.replace(
                f'<keyName>{_TEMPLATE_TARGET_KEYNAME}</keyName>',
                f'<keyName>{tgt_kn}</keyName>',
                1,
            )
            new_main = _blob_from_xml(xml)
            CHUNK = 512
            new_chunks = [new_main[i:i+CHUNK] for i in range(0, len(new_main), CHUNK)]
            ...
            for chunk_idx, chunk in enumerate(new_chunks):
                base_idx = [342, 351, 360, 369][chunk_idx]
                first = b'\x02\x08\x06\x01' + struct.pack('<H', len(chunk)) + b'\x00\x00' + chunk[:56]
                overrides[base_idx] = first.ljust(64, b'\x00')
                for j, off in enumerate(range(56, len(chunk), 60)):
                    cp = b'\x02\x08\x07\x01' + chunk[off:off+60]
            ...
            # Helper: override specific byte in a 06 01 packet's data portion
            # data starts at packet offset 8 (2 rid+size + 2 cmd + 2 size + 2 pad)
            def override_byte(seq_idx, data_byte_idx, new_val):
                p = bytearray(seq[seq_idx][1])
                p[8 + data_byte_idx] = new_val
                overrides[seq_idx] = bytes(p)

            override_byte(378, 13, code)    # 616d byte 13
            override_byte(383, 8, b626)     # 626d byte 8
            override_byte(403, 8, b666)     # 666d byte 8
            override_byte(408, 8, b676)     # 676d byte 8
            override_byte(413, 4, b686)     # 686d byte 4

            # 0700 manifest byte 2
            mf = bytearray(seq[439][1])
            mf[8 + 2] = code
            overrides[439] = bytes(mf)

            print(f'Overriding {len(overrides)} packets for {tgt_kn}. Replaying...')
            _replay_p1_sequence(fd, overrides=overrides)
            print(f'Done. Restart controller, press P1 paddle, report result.')
        elif args.cmd == 'bootstrap-p1-full-target':
            # Override target-dependent bytes across captured slots:
            #   616d byte 13 = target code
            #   0700 byte 2  = target code (same value)
            #   606d XML keyName
            # (626/666/676/686 bytes appear to track target FAMILY —
            # LB matches A, RB matches X — not per-target, so treated as
            # family metadata and left at A-capture defaults.)
            tgt_kn = TARGET_KEYS.get(args.target.upper(), args.target)
            code = _ACTION_HDR_TARGET_CODES.get(tgt_kn)
            if code is None:
                print(f'Target {args.target} has no known code', file=sys.stderr)
                sys.exit(1)
            ...
            # 606d chunk overrides
            chunk_start_indices = [342, 351, 360, 369]
            for chunk_idx, chunk in enumerate(new_chunks):
                base_idx = chunk_start_indices[chunk_idx]
                size_hdr = struct.pack('<H', len(chunk))
                first_pkt = b'\x02\x08\x06\x01' + size_hdr + b'\x00\x00' + chunk[:56]
                first_pkt = first_pkt.ljust(64, b'\x00')
                overrides[base_idx] = first_pkt
                for j, off in enumerate(range(56, len(chunk), 60)):
                    cont_data = chunk[off:off+60]
                    cont_pkt = b'\x02\x08\x07\x01' + cont_data
                    cont_pkt = cont_pkt.ljust(64, b'\x00')
                    overrides[base_idx + 1 + j] = cont_pkt

            # 616d byte 13 override (index 378 in sequence, data byte offset 8+13=21)
            orig_616 = bytearray(seq[378][1])
            orig_616[8 + 13] = code
            orig_616[8 + 14] = 0
            orig_616[8 + 15] = 0
            orig_616[8 + 16] = 0
            ...
            # Find 0700 write index — it's the ONLY 06 01 with size=71
            manifest_idx = None
            for i, (_d, pkt) in enumerate(seq):
                if len(pkt) >= 6 and pkt[2] == 0x06 and pkt[3] == 0x01:
                    sz = struct.unpack_from('<H', pkt, 4)[0]
                    if sz == 71:
                        manifest_idx = i
                        break
            if manifest_idx is None:
                print('ERROR: could not find 0700 manifest write in sequence')
                sys.exit(1)
            # Manifest 71B split: 56 in 06 01, 15 in one 07 01
            # Byte 2 of manifest data is at packet offset 8+2 = 10
            orig_mf = bytearray(seq[manifest_idx][1])
            orig_mf[8 + 2] = code
            overrides[manifest_idx] = bytes(orig_mf)
            print(f'Manifest write at seq idx {manifest_idx}, overriding byte 2 to 0x{code:02x}')

            print(f'Overriding {len(overrides)} packets for {tgt_kn}. Replaying...')
            _replay_p1_sequence(fd, overrides=overrides)
            print(f'Done. Restart controller, press P1 paddle, report result.')
        elif args.cmd == 'bootstrap-p1-xml-target':
            tgt_kn = TARGET_KEYS.get(args.target.upper(), args.target)
            code = _ACTION_HDR_TARGET_CODES.get(tgt_kn)
            if code is None:
                print(f'Target {args.target} has no known action_hdr code', file=sys.stderr)
                sys.exit(1)

            # Decompress the 606d blob, swap keyName, recompress
            orig_main = _load_p1_bootstrap_bundle()['606d']
            xml = zlib.decompress(orig_main[7:]).decode('utf-8', errors='replace')
            xml = xml.replace(
                f'<keyName>{_TEMPLATE_TARGET_KEYNAME}</keyName>',
            ...
            new_main = _blob_from_xml(xml)
            print(f'New 606d blob: {len(new_main)}B (orig {len(orig_main)}B)')

            # Split into chunks: 3 x 512 + remainder
            CHUNK = 512
            new_chunks = [new_main[i:i+CHUNK] for i in range(0, len(new_main), CHUNK)]
            orig_chunk_count = 4  # 512+512+512+252
            if len(new_chunks) != orig_chunk_count:
                print(f'WARNING: chunk count differs ({len(new_chunks)} vs {orig_chunk_count})')

            # Build override packets for 606d chunks.
            # Sequence indices for 606d 06 01 starts: 342, 351, 360, 369.
            # Each 06 01 followed by 07 01 continuations to fill chunk.
            # 512B chunk = 1 x 06 01 (56B) + 8 x 07 01 (60B), last pads.
            # 252B chunk = 1 x 06 01 (56B) + 4 x 07 01 (remaining 196B).
            seq = _load_p1_full_sequence()
            overrides = {}
            chunk_start_indices = [342, 351, 360, 369]
            ...
            for chunk_idx, chunk in enumerate(new_chunks):
                if chunk_idx >= len(chunk_start_indices):
                    print(f'ERROR: more chunks than skeleton supports, cannot handle')
                    sys.exit(1)
                base_idx = chunk_start_indices[chunk_idx]
                # 06 01 packet: 02 08 06 01 <size_LE> 00 00 <first 56 of chunk>
                size_hdr = struct.pack('<H', len(chunk))
                first_pkt = b'\x02\x08\x06\x01' + size_hdr + b'\x00\x00' + chunk[:56]
                first_pkt = first_pkt.ljust(64, b'\x00')
                overrides[base_idx] = first_pkt
                # 07 01 continuation packets
                cont_offsets = list(range(56, len(chunk), 60))
                for j, off in enumerate(cont_offsets):
                    cont_data = chunk[off:off+60]
                    cont_pkt = b'\x02\x08\x07\x01' + cont_data
                    cont_pkt = cont_pkt.ljust(64, b'\x00')
                    overrides[base_idx + 1 + j] = cont_pkt

            # Also override 616d byte-13
            orig_616d_pkt = seq[378][1]
            new_616d = bytearray(orig_616d_pkt)
            new_616d[8 + 13] = code
            new_616d[8 + 14] = 0
            new_616d[8 + 15] = 0
            new_616d[8 + 16] = 0
            overrides[378] = bytes(new_616d)

            print(f'Overriding {len(overrides)} packets. Replaying...')
            _replay_p1_sequence(fd, overrides=overrides)
            print(f'Done. Restart controller, press P1 paddle, report result.')
        elif args.cmd == 'bootstrap-p1-test-target':
            # Diagnostic: replay p1_to_A sequence but change ONLY the 616d
            # action_hdr byte-13 code. Leaves XML at P1→A. If physical paddle
            # press after restart triggers the requested TARGET instead of A,
            # action_hdr is the dispatch source. If still A, XML is.
            tgt_kn = TARGET_KEYS.get(args.target.upper(), args.target)
            code = _ACTION_HDR_TARGET_CODES.get(tgt_kn)
            if code is None:
                print(f'Target {args.target} has no known action_hdr byte-13 code', file=sys.stderr)
                sys.exit(1)
            # Load captured 616d packet at sequence index 378, swap byte 13,
            # rebuild full HID packet.
            # The packet is: 02 08 06 01 <size_LE:2> 00 00 <22 data bytes> <pad to 64>
            # Data bytes: 0b b1 01 01 00 00 00 00 01 60 6d 02 01 <code:4> 02 00 00 00 00
            seq = _load_p1_full_sequence()
            orig_pkt = seq[378][1]  # (delay, pkt)
            new_data = bytearray(orig_pkt)
            # byte 13 of the 22B data starts at HID packet offset 8+13 = 21
            new_data[8 + 13] = code
            # byte 14-16 (remaining bytes of LE uint32) should be 0
            new_data[8 + 14] = 0
            new_data[8 + 15] = 0
            new_data[8 + 16] = 0
            overrides = {378: bytes(new_data)}
            print(f'Replaying with action_hdr override: target={tgt_kn} code=0x{code:02x}')
            _replay_p1_sequence(fd, overrides=overrides)
            print(f'Done. Restart controller, press P1 paddle, report what it triggers.')
